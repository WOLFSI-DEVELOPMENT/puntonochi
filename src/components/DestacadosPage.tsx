...
export function DestacadosPage({ onClose, onSelectBusiness }: { onClose: () => void, onSelectBusiness: (place: Place) => void }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      ck.applyAll('.ck-card', { radius: 23, smoothing: 1 });
      ck.applyAll('.ck-card-inner', { radius: 18, smoothing: 1 });
    }, 600);
        ...
        <div className="grid grid-cols-2 gap-4">
          {places.map((place, i) => (
            <motion.div 
              ...
              className="ck-card rounded-[23px] bg-white flex flex-col cursor-pointer active:scale-95 transition-transform p-[5px]"
            >
              <div className="ck-card-inner rounded-[18px] aspect-square relative bg-neutral-100 overflow-hidden">
