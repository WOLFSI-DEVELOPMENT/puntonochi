...
const index = code.indexOf('.bg-card-yellow');
const cleanEndIndex = code.indexOf('}', code.indexOf('}', index) + 1) + 1;
let cleanCode = code.substring(0, cleanEndIndex);
...
`;
fs.writeFileSync('src/index.css', cleanCode);
